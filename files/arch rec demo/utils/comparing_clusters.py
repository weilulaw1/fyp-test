...
def fun_a2a_2(result, result_gt):
    result_clusters = {}
    for i, r in enumerate(result):
        try:
...
def compare_two_metrics(metrics1, metrics2)->dict:
    metric_lead = {}
    for metric in metrics1:
        v1 = metrics1[metric]
        v2 = metrics2.get(metric, 0)
        if not v1 or not v2:
            metric_lead[metric] = None
        else:
            metric_lead[metric] = (v1 - v2)/v2
    metric_lead['avg'] = sum([v for v in metric_lead.values() if v != None])/len([v for v in metric_lead.values() if v != None])
    return metric_lead
