    ...
    with open(fn, 'r') as fp:
        for l in fp:
            ...
            for f in filenames.split():
                f = f.rstrip(',')
                result_dict[f] = cluster_name
    result_dict = remap_result_dict(result_dict)
    return result_dict

def rsf2resdict(rsf_fn):
    result_dict = {}
    with open(rsf_fn, 'r') as fp:
        for l in fp:
            a, cluster_name, filename = l.split()
            result_dict[filename] = cluster_name
    result_dict = remap_result_dict(result_dict)
    return result_dict

def get_turbo_mq_weighted(nx_graph, result_dict):
    # assert len(nx_graph) == len(result_dict)
    for file in result_dict:
        assert file in nx_graph
    nx_graph = nx_graph.subgraph(result_dict.keys())
    # basic
