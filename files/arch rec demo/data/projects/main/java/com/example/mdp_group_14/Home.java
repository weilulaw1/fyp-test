...
//        pathTranslator.translatePath("MOVE,FORWARD,30");
    ...
    public static void printMessage(String type, String message) {
        ...
        try {
            jsonMessage.put("cat", type);
            jsonMessage.put("value", message);  // The actual message value
        ...
        editor = sharedPreferences.edit();
        editor.putString("last_message", message);  // Store the last message
        editor.apply();  // Apply changes

        // Check Bluetooth connection and send the message
        if (BluetoothConnectionService.BluetoothConnectionStatus) {
            byte[] bytes = jsonString.getBytes(Charset.defaultCharset());
            BluetoothConnectionService.write(bytes);  // Send the JSON string
        }
...
//        BluetoothCommunications.getMessageReceivedTextView().append("ROBOT,"+ (x-1) +"," + (y-1) + "," + dir+"\n"); //for troubleshooting
    ...
    BroadcastReceiver messageReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            String cleanMessage = message.trim(); // Trims the string
...
// Inside your onReceive method
            if (cleanMessage.startsWith("{") && cleanMessage.endsWith("}")) {
                try {
                    JSONObject jsonObject = new JSONObject(cleanMessage);
// "command"
                    if (jsonObject.has("cat") && jsonObject.getString("cat").equals("TARGET")) {
                        Log.d(TAG,"Raw Message Received: "+cleanMessage);
                        try {
                            // Extract the TARGET array from the JSON object
                            JSONArray targetArray = jsonObject.getJSONArray("value");


                            // Ensure that the array has exactly two elements (obstacle number and target ID)
                            if (targetArray.length() == 2) {
                                int obstacleNumber = targetArray.getInt(0); // First value: obstacle number
                                String targetID = targetArray.getString(1); // Second value: target ID as string
                                ...
                                // Update the gridMap using the obstacle number and target ID
                                gridMap.updateIDFromRpi(String.valueOf(obstacleNumber - 1), targetID);
                                obstacleID = String.valueOf(obstacleNumber - 2); // Set obstacleID based on obstacleNumber

                                // Optional: Check for specific commands in targetID
                                if (targetID.contains("STOP")) {
                                    // Handle the "STOP" command
                                    String[] temp1 = targetID.split(" ");
                                    if (temp1.length > 0) {
                                        obstacleID = temp1[0]; // Process the first element after STOP
                                    }
                                }
                            } else {
                                Log.d("MainActivity", "Invalid TARGET array length");
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
                        } catch (Exception e) {
                    ...
                    if (jsonObject.has("cat")) {
                        try {
                            String catValue = jsonObject.getString("cat");

                            if (catValue.equals("commands")) {
                                String commandsValue = jsonObject.getString("value");
                                // Accumulate message parts
                                DataManager.getInstance().
                                        appendToMessageBuffer(commandsValue);

                                // Try processing the complete message
                                try {
                                    String completeMessage = DataManager.getInstance().getBufferedMessage();
                                    // Replace single quotes with double quotes for valid JSON
                                    completeMessage = completeMessage.replace("'", "\"");
                                    Log.d(TAG, "Complete message: " + completeMessage);

                                    // Try parsing the JSON array
                                    robotPositions = new JSONArray(completeMessage);
                                    DataManager.getInstance().setRobotPositions(robotPositions);
                                    DataManager.getInstance().clearMessageBuffer(); // Clear buffer once done
                                } catch (JSONException e) {
                                    Log.e(TAG, "JSONException: message may not be complete yet.");
                                }
                            }
                        } catch (JSONException e) {
                    ...
                    showLog("Error processing message: " + e.getMessage());
                }
            }

// Add your existing processGridData method here
...
//                showLog("received Stop");
    ...
    private void updateRobotPosition(int xCoord, int yCoord, int directionValue) {
        // Convert directionValue to string and update gridMap as needed
        String direction = "";
